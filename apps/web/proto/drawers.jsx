                ...
                return (
                  <div key={g.subcap_id || gi} className="card-tile" style={{ padding: "8px 10px", marginBottom: 6 }}>
                    ...
                    {/* The catalogue path ends in the L4 feature and the cell
                        ...
                        It also carries the producer's vote count mid-string —
                        `… > [L3-SF-DC-CORE] Data Cloud (count: 3) > Data
                        Stream …` — so the same strip the subtitle makes is
                        made here, behind the disclosure where the reader
                        expands it (P-05). */}
                    {dwText(g.catalogue_path) ? (
                      <div style={{ fontSize: 9.5, color: "var(--z-muted)", marginTop: 3, lineHeight: 1.4, wordBreak: "break-word" }}>
                        {dwText(g.catalogue_path).replace(/\s*\(count:\s*\d+\)/g, "")}
                      </div>
                    ) : null}
                    {dwText(g.gap) ? (
...
const IP_SKIP_SEGMENTS = new Set(["r_layer", "probes_run", "sources_searched",
                                  ...
                                  "redacted_paths", "redacted_count", "redaction_note", "annotation"]);
...
/* Which promoted SECTION each adapted key came from.
   ...
   ids anyway. */
const IP_SECTION_OF = {
  exec_summary: "overview.exec_summary", framing: "overview.scores",
  posture_basis: "overview.scores", narrative_thread: "overview.scores",
  scores: "overview.scores", findings: "overview.findings",
  opportunity: "overview.opportunity", opportunityTiles: "overview.opportunity",
  oss: "overview.opportunity", firmographics: "overview.firmographics",
  whyNow: "overview.why_now", leadership: "overview.leadership",
  thoughtLeadership: "overview.thought_leadership",
  financials: "overview.financial_series", sentiment: "overview.sentiment",
  uncertainty: "overview.ceilings", coverage: "overview.evidence_coverage",
  insightCards: "insights.insights", landscape: "insights.landscape",
  platformStory: "platform.platform_story",
  recommendations: "platform.recommendations", starters: "platform.starters",
  roadmap: "platform.roadmap", roadmapBasis: "platform.roadmap",
  stairstep: "platform.stairstep", stairstepClusters: "platform.stairstep",
  workbookScores: "heatmap.workbook_scores", focusAreas: "heatmap.focus_areas",
  cellEvidence: "heatmap.cell_evidence", alerts: "heatmap.alerts",
  caps: "heatmap.safeguard_gates", gates: "heatmap.safeguard_gates",
  evidenceAge: "heatmap.evidence_age", cohorts: "heatmap.cohort_patterns",
  valueChain: "heatmap.value_chain", valueChains: "heatmap.value_chain",
  timeline: "context.timeline", timelineMeta: "context.timeline",
  issues: "context.issue_register", regulatory: "context.regulatory_standing",
  contextSentiment: "context.context_sentiment",
  acquisitions: "context.acquisitions", techStack: "techstack.techstack",
...
function ipRank(question, passages, limit) {
  ...
  for (let i = 0; i < passages.length; i++) {
    ...
    scored.push({ score, substance: Math.min(words.size, IP_SUBSTANCE_CAP), i, p });
  }
...
function liveSurfaceMessages(surface, ctx) {
  ...
  if (surface === "platform_story") {
    ...
    const areasOf = (p) => [...new Set((p.gaps || [])
      .map(g => { const s = dwText(g.l3_area); return s ? s.replace(/\s*\(count:\s*\d+\)/g, "").trim() : null; })
      .filter(Boolean))];
