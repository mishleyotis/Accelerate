...
function OvStorylineChallenge({ challenge, audience }) {
  ...
  return (
    <div style={{ marginTop: 14, borderTop: "1px solid var(--z-sep)", paddingTop: 12 }}>
      <div className="row" style={{ gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
        <span className="eyebrow" style={{ fontSize: 9.5, color: "var(--z-dpur)" }}>
          Storyline challenge
        </span>
        <span style={{ fontSize: 11, color: "var(--z-muted)" }}>
          {volleys.length} {volleys.length === 1 ? "volley" : "volleys"} · the objections this
          story was tested against, and what it did with each
        </span>
      </div>
      {volleys.map((v, i) => {
        const isOpen = open === i;
        ...
        return (
          <div key={v.volley != null ? v.volley : i}
               style={{ borderTop: i ? "1px solid var(--z-sep)" : 0, padding: "8px 0" }}>
            <button onClick={() => setOpen(o => o === i ? null : i)}
                    style={{ display: "flex", alignItems: "flex-start", gap: 8, width: "100%",
                             background: "none", border: 0, padding: 0, cursor: "pointer",
                             textAlign: "left" }}>
              <span className="f-mono" style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: ".06em",
                                                textTransform: "uppercase", color: "var(--z-dpur)",
                                                background: "rgba(115,91,161,.14)", borderRadius: 4,
                                                padding: "2px 7px", flexShrink: 0, whiteSpace: "nowrap" }}>
                {who(v)}
              </span>
              <span style={{ flex: 1, minWidth: 0, fontSize: 12.5, color: "var(--z-dark)", lineHeight: 1.5 }}>
                {asText(v.challenge)}
              </span>
              {v.outcome ? (
                <span className={`b ${held ? "b-muted" : "b-org"}`} style={{ flexShrink: 0 }}>
                  {held ? "Story held" : "Story changed"}
                </span>
              ) : null}
              <Icon name={isOpen ? "chevron-u" : "chevron-d"} size={13}
                    style={{ color: "var(--z-muted)", flexShrink: 0, marginTop: 2 }} />
            </button>
            {isOpen ? (
              <div style={{ marginTop: 8, paddingLeft: 4 }}>
                {asText(v.answer) ? (
                  <div style={{ marginBottom: 8 }}>
                    <div style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: ".08em",
                                  color: "var(--z-muted)", textTransform: "uppercase",
                                  marginBottom: 2 }}>Answer</div>
                    <div style={{ fontSize: 12.5, color: "var(--z-body)", lineHeight: 1.6 }}>{asText(v.answer)}</div>
                  </div>
                ) : null}
                {asText(v.changed) ? (
                  <div style={{ background: "var(--z-lav)", borderLeft: "3px solid var(--z-dpur)",
                                borderRadius: "0 6px 6px 0", padding: "8px 12px" }}>
                    <div style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: ".08em",
                                  color: "var(--z-dpur)", textTransform: "uppercase",
                                  marginBottom: 2 }}>What changed</div>
                    <div style={{ fontSize: 12, color: "var(--z-body)", lineHeight: 1.55 }}>{asText(v.changed)}</div>
                  </div>
                ) : null}
              </div>
            ) : null}
...
function SCQABody({ entity, expanded, openEvidence, audience }) {
  ...
  return (
    <div style={{ fontSize: 14, color: "var(--z-dark)", lineHeight: 1.7, maxWidth: 880 }}>
      ...
      {expanded ? (
        <OvStorylineChallenge challenge={s.storyline_challenge} audience={audience} />
      ) : null}
    </div>
  );
}
