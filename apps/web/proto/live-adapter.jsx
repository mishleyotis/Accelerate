...
const PLATFORM_ALIASES = {
  salesforce: "SF", "data cloud": "SF", agentforce: "SF", fsc: "SF",
  "financial services cloud": "SF", "marketing cloud": "SF",
  "service cloud": "SF", "sales cloud": "SF", crma: "SF",
  databricks: "DB", "mosaic ai": "DB", lakeflow: "DB", lakehouse: "DB",
  tableau: "TBL", pulse: "TBL",
  twilio: "TW", engage: "TW", conversations: "TW",
  ncino: "nCino", "workflow engine": "nCino", "document manager": "nCino",
};
...
function cagrOf(series) {
  ...
  const pts = (series || [])
    .map((s) => ({ y: yearOf(s), v: num(s.value) }))
    .filter((p) => p.y !== null && p.v !== null && p.v > 0)
    .sort((a, b) => a.y - b.y);
  ...
  return { cagr, cagr_basis: `${a.y}–${b.y} · ${years} yr` };
}
...
const BAND_MIDPOINT = { activating: 1.5, building: 2.5, competing: 3.5,
                        differentiating: 4.5 };
...
function adaptUncertainty(ceilings) {
  ...
  for (const r of rows) {
    ...
    const word = asNum === null
      ? BAND_MIDPOINT[String(r.ceiling || "").trim().toLowerCase()] ?? null
      : null;
    out[r.category_id] = {
...
function adaptEvidenceSummary(evidenceEnvelope) {
  ...
  if (!d) return null;
  const facts = Object.keys(d.claims || {}).reduce(
    (a, k) => a + (d.claims[k] || 0), 0);
  return {
...
function adaptWhyNow(whyNow) {
  ...
  return signals.map((s) => ({
    ...
    evidence: s.e_ids || [],
    timeline: s.dated_on ? { date: s.dated_on, event: s.dated_source || null } : null,
...
const NOT_A_SENTENCE_END = new RegExp(
  '(?:^|[\\s("“.])(?:[A-Za-z]|Mr|Mrs|Ms|Dr|Prof|Rev|Hon|Sr|Jr|St|Inc|Corp'
  + '|Co|Ltd|LLC|LLP|plc|Bros|Assn|Dept|Univ|No|vs|etc|approx|Fig'
...
function techLayersOf(techstack) {
  ...
  const flagged = rows.filter(r => promoted.get(r.layer)
                                   && promoted.get(r.layer).is_primary_gap);
  if (flagged.length === 1) {
...
function adaptFocusAreas(focus) {
  return ((focus && focus.focus_areas) || []).map((f) => ({
    ...
    source: { type: f.source_document || null, page: f.source_page || null,
              doc: f.source_filename || null },
    entity_score: num(f.entity_score), peer_score: num(f.peer_score),
    delta: num(f.delta),
...
const EFFECT_TOKEN_LABEL = {
  ADVANCED: "POSITIVE", CONSTRAINED: "NEGATIVE", NEUTRAL: "NEUTRAL",
};
...
/* `timeline.arc_shape` has a closed vocabulary of five (Surface Specification,
   ...
   contract words. */
const ARC_SHAPES = ["STEADY_INVESTMENT", "STOP_START", "POST_EVENT_CATCHUP",
...
function splitMaturityEffect(raw) {
  ...
  if (!m) return { token: null, reason: s };
  ...
  const looksLikeToken = /^[A-Z_]+$/.test(m[1].trim())
    || MATURITY_EFFECT_TOKENS.includes(token);
  if (!looksLikeToken) return { token: null, reason: s };
  return { token, reason: m[2].trim() || null };
}
...
function buildLiveEntity(entityId, pages, extras) {
  const p = pages || {};
  const x = extras || {};
  const overview = p.overview, heatmap = p.heatmap, insights = p.insights,
        platform = p.platform, context = p.context, techstack = p.techstack;
  ...
  return {
    ...
    enrichment: {
      ...
      thought_leadership:
        (secOf(overview, "thought_leadership") || {}).enrichment_status || null,
      sentiment: (secOf(overview, "sentiment") || {}).enrichment_status || null,
      firmographics:
        (secOf(overview, "firmographics") || {}).enrichment_status || null,
    },
    ...
    for (const name of Object.keys(secs)) {
      ...
      out[`${page}.${name}`] = {
        ...
        redacted_count: (s.redacted_count
                         ?? (s.redacted_paths ? s.redacted_paths.length : null)),
...
Object.assign(window, {
  ...
  adaptRoadmap, adaptStairstep, adaptEvidence, adaptAnswers,
  platformChips, scaleMaxOf, tileScaleMaxOf,
