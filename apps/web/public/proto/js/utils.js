...
function humanText(v, keys) {
  ...
  if (obj !== null) {
    if (Array.isArray(obj)) {
      ...
      out = null;
      for (const k of keys || ["title", "statement", "text", "label", "name", "summary", "headline", "value"]) {
...
const EVIDENCE_CITATION_GROUP = new RegExp(String.raw`\s*[\(\[](?:\s*${EVIDENCE_ID}\s*[,;]?)+[\)\]]`, "g");
function clientProse(value) {
  const t = typeof value === "string" ? value : asText(value);
  if (!t) return t;
  return t.replace(EVIDENCE_CITATION_GROUP, "").replace(/\s+([,.;:!?])/g, "$1").replace(/\(\s*\)|\[\s*\]/g, "").replace(/\s{2,}/g, " ").trim();
}
...
function absenceBadge(section) {
  ...
  const kind = (state && state.empty_state || {}).kind || state && state.data_source || null;
  return {
...
function sessionUser() {
  ...
  const name = live.name || email.split("@")[0].split(/[._-]+/).filter(Boolean).map(w => w[0].toUpperCase() + w.slice(1)).join(" ") || email;
...
function useLiveEntity(displayId, audience, runId, actingRole) {
  ...
  useEffect(() => {
    ...
    Promise.all([...pages.map(p => get(`/api/entity/${id}/${p}?${qs()}`)), get(`/api/entity/${id}/evidence?${qs()}`), get(`/api/entity/${id}/subcaps?${qs()}`)]).then(results => {
      ...
      pages.forEach((p, i) => {
        if (results[i].ok) {
          byPage[p] = results[i].body;
          return;
        }
        if (results[i].status === 403) {
          withheld[p] = results[i].body && results[i].body.detail || "this dashboard is not available to your role or audience";
        }
      });
...
function fmtPct(n) {
  return n == null || n === "" || !isFinite(Number(n)) ? null : `${(Number(n) * 100).toFixed(0)}%`;
}
...
function entityMatches(e, q) {
  ...
  if (!ql) return true;
  return [e && e.name, e && e.domain, e && e.id, e && e.slug].some(v => v != null && String(v).toLowerCase().includes(ql));
}
function relTime(s) {
  if (!s) return "-";
