...
function adaptFinancials(financialSeries, firmographics, regulatory) {
  ...
  const scaled = scaleMoneySeries(series.map(s => s.value), (series.find(s => s.unit) || {}).unit);
...
function adaptStairstep(stairstep) {
  ...
  return {
    ...
    steps: steps.map(s => ({
      ...
      blocking_refs: (s.blocking_findings || []).map(b => parseMaybeJSON(b) || (typeof b === "string" ? {
        title: b
      } : null)).filter(Boolean),
...
function buildLiveEntity(entityId, pages, extras) {
  ...
  return {
    ...
    peerSets: peerSetsOf(secOf(techstack, "techstack"), (overview && overview.entity || heatmap && heatmap.entity || {}).sub_vertical),
