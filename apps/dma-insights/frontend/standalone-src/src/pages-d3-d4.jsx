...
function ClientPlatform({ entity, run }) {
  ...
  const { setIpSurface, setIpContext, setIpOpen, openEvidence } = useApp();
    ...
    <div>
      <div className="page-head">
        ...
        <div className="actions">
          <button className="btn btn-tertiary"><Icon name="download" size={13} /> Roadmap export</button>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 16, marginBottom: 16 }}>
            ...
            <table className="tbl">
              ...
              <tbody>
                {gaps.slice(0, 10).map(s => (
                  <tr key={s.id}>
                    <td><div style={{ fontSize: 12, fontWeight: 500 }}>{s.name}</div><div className="f-mono" style={{ fontSize: 10, color: "var(--z-muted)" }}>{s.id}</div></td>
                    <td><span className="b b-purple">{s.pillar}</span></td>
                    <td><MaturityChip score={s.score} /></td>
                    <td><MaturityChip score={s.peerMedian} /></td>
                    <td><span style={{ fontFamily: "var(--font-mono)", color: "var(--z-below)" }}>−{(s.peerMedian - s.score).toFixed(1)}</span></td>
                    <td style={{ fontSize: 11, color: "var(--z-body)" }}>{recs.find(r => r.platform === platform)?.feature || "Platform"}</td>
                  </tr>
                ))}
        ...
        <div className="card">
          ...
          {prerequisites[platform].map(p => (
            <div key={p.id} style={{ padding: "10px 0", borderBottom: "1px solid var(--z-sep)" }}>
              ...
              <div style={{ fontSize: 11, color: "var(--z-muted)" }}>Min {p.min.toFixed(1)} · Current {p.current.toFixed(1)}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        <div className="card flush">
          ...
          <div>
            {recs.filter(r => r.platform === platform).map(r => (
              <div key={r.id} style={{ padding: "12px 18px", borderBottom: "1px solid var(--z-sep)" }}>
                <div className="row" style={{ marginBottom: 4 }}>
                ...
                <div style={{ fontSize: 11.5, color: "var(--z-body)", lineHeight: 1.55, margin: "6px 0" }}>
                  Root cause: {r.root_cause.map(eid => <button key={eid} className="chip" style={{ marginRight: 4 }} onClick={() => openEvidence(eid)}>{eid}</button>)}
                </div>
        ...
        <div className="card flush">
          <div className="card-head"><h3>Conversation starters</h3><button className="btn btn-tertiary btn-sm"><Icon name="copy" size={12} /> Copy all</button></div>
            ...
            {conversationStarters[platform].map((cs, i) => (
              <div key={i} style={{ padding: 10, marginBottom: 8, background: "var(--ph0-lt)", border: "1px solid var(--ph0-bd)", borderRadius: 8 }}>
                <div className="row" style={{ marginBottom: 6 }}>
                  ...
                  <span className="spacer" />
                  <button className="btn btn-tertiary btn-sm" style={{ color: "var(--z-dpur)" }}><Icon name="copy" size={11} /></button>
                </div>
    ...
    <div className="card" style={{ marginBottom: 16 }}>
      <div className="row" style={{ marginBottom: 14 }}>
        ...
        <div className="toggle-row">
          {Object.entries(DMA.STAIRSTEP_CLUSTERS).map(([k, v]) => (
            <button key={k} className={cluster === k ? "on" : ""} onClick={() => setCluster(k)}>{v.label}</button>
          ))}
        </div>
      </div>
...
function TransformationRoadmap({ entity }) {
  const { openEvidence, openRec } = useApp();
  ...
  return (
    <div className="card" style={{ marginBottom: 16 }}>
      <div className="row" style={{ marginBottom: 16 }}>
        ...
        <button className="btn btn-tertiary btn-sm"><Icon name="download" size={11} /> Export</button>
      </div>
      ...
      <div className="co co-teal" style={{ marginTop: 14 }}>
        ...
        <div>
          ...
          <div className="co-body">Foundation (P4C1) must reach M3 before Phase 2 can deliver on Marketing + Engage. Mosaic AI in Phase 3 requires the Lakehouse from Phase 1 to be operational. Phases overlap - staffing is concurrent, not sequential.</div>
        </div>
      </div>
    </div>
  );
