...
/**
 * BackendErrorBanner — surfaces 5xx / network failures from the
 * backend-loader. Reads window.DMA_LOAD_STATE.errors (set by
 * backend-loader.js:_pushError) and listens to the `dma:load-error`
 * CustomEvent for live updates.
 *
 * Show when errors.length > 0. Dismiss button clears the list (but
 * a fresh failure repopulates it). The banner is operator-facing —
 * lets the AE distinguish "real empty state" from "backend silently
 * dropped my data".
 */
function BackendErrorBanner() {
  const [errors, setErrors] = useState(
    () => (window.DMA_LOAD_STATE?.errors || []).slice(),
  );
  const [dismissed, setDismissed] = useState(false);

  useEffect(() => {
    function refresh() {
      // 2026-05-29 QA audit P1: only show GLOBAL-scoped errors here.
      // Admin/diagnostic endpoint failures (scope==="admin") have their
      // own per-page renderers and were polluting the app-wide banner
      // ("Backend data failed to load... 500 /api/v1/admin/import-audit
      // /by-entity" on every non-admin page). Treat missing scope as
      // "global" for back-compat with anything still calling _pushError
      // without an explicit scope.
      const all = (window.DMA_LOAD_STATE?.errors || []);
      const next = all.filter(e => (e.scope || "global") === "global");
      setErrors(next);
      if (next.length > 0) setDismissed(false);
    }
    window.addEventListener("dma:load-error", refresh);
    // Also poll once per 5s in case the event was missed (chrome
    // mounted after the error fired during boot hydrate).
    const t = setInterval(refresh, 5000);
    refresh();
    return () => {
      window.removeEventListener("dma:load-error", refresh);
      clearInterval(t);
    };
  }, []);
  ...
  const allPaths = [...new Set(errors.map(e => e.path))];
  ...
  return (
    <div
      ...
      style={{
        ...
        zIndex: 1000,
        padding: "10px 16px",
        // Use existing tokens (--z-org-lt for soft background;
        // --z-org for border / --z-below for text accent). Fallbacks
        // ensure the banner remains readable if tokens.css is missing.
        background: "var(--z-org-lt, #fff4ec)",
        borderBottom: "1px solid var(--z-org, #FE9732)",
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        <strong>Backend data failed to load.</strong>{" "}
        Some pages may show empty content. Latest error:{" "}
        <code style={{ background: "rgba(0,0,0,0.05)", padding: "2px 6px", borderRadius: 4 }}>
          {latest.status ? `${latest.status} ` : ""}{latest.path}
        </code>
        {allPaths.length > 1 ? (
          <span style={{ opacity: 0.7 }}> (+{allPaths.length - 1} other endpoint{allPaths.length > 2 ? "s" : ""})</span>
        ) : null}
      </div>
      <button
        className="btn btn-tertiary btn-sm"
        onClick={() => {
          window.DMA_LOAD_STATE.errors = [];
          setDismissed(true);
        }}
...
function Sidebar() {
  const { route, role, openAlerts, activeRuns, signOut, sidebarOpen, setSidebarOpen, user, pushToast } = useApp();
  ...
  // `dim` no longer silently swallows the click — it surfaces a toast
  // explaining WHY the page is gated. Previously AE users clicking
  // Alerts got no feedback, looking like the whole sidebar was broken.
  const NavItem = ({ href, icon, label, badge, dim, dimReason, dot, indent }) => (
    <button
      className={`sb-a ${isOn(href) ? "on" : ""} ${dim ? "dim" : ""}`}
      onClick={() => dim
        ? pushToast(dimReason || `${label} is not available for your role`, "warn")
        : go(href)}
      title={dim ? (dimReason || "Not available for your role") : null}
      ...
      <aside className={`sb ${sidebarOpen ? "open" : ""}`}>
        ...
        <nav className="sb-nav">
          ...
          <NavItem href="/alerts"      icon="bell"     label="Alerts"
            ...
            dim={role === "AE"}
            dimReason="Alerts is Analyst+ only — switch role via Settings → Acting as." />
        ...
        <div className="sb-foot">
          ...
          <div className="sb-foot-meta">
            <div className="sb-foot-name" title={user?.email || ""}>{user?.name || "Not signed in"}</div>
            <div className="sb-foot-role">{role}</div>
          </div>
          <button className="icon-btn" style={{ color: "rgba(255,255,255,.6)" }} title="Sign out" onClick={() => { signOut(); navigate("/login"); }}>
...
function TopBar({ title, crumbs, right }) {
  ...
  const searchResults = useMemo(() => {
    ...
    // Insight + evidence search results route to the parent entity's
    // page; the entity id lives on each card / evidence row once wired.
    const insights = DMA.INSIGHT_CARDS.filter(c => c.title.toLowerCase().includes(ql) || c.id.toLowerCase().includes(ql)).slice(0, 3)
      .map(c => ({ kind: "insight", title: c.title, sub: `${c.id} · ${c.flag}`, route: c.entity_id ? `/clients/${c.entity_id}/insights?card=${c.id}` : "/clients", icon: "insight" }));
    const evidence = DMA.EVIDENCE.filter(e => e.title.toLowerCase().includes(ql) || e.id.toLowerCase().includes(ql)).slice(0, 3)
      .map(e => ({ kind: "evidence", title: e.title, sub: `${e.id} · ${e.tier}`, route: e.entity_id ? `/clients/${e.entity_id}/insights?evidence=${e.id}` : "/clients", icon: "evidence" }));
  ...
  return (
    <div className="popover">
      <div className="popover-head">
        ...
        <button className="btn btn-tertiary btn-sm">Mark all read</button>
      </div>
      <div className="popover-body">
...
function SettingsPopover({ onClose }) {
  const { role, setRole, audience, setAudience, signOut, user } = useApp();
  const canActAs = user?.can_act_as || ["AE"];
  const items = [
    { label: "Profile",          icon: "user",     route: "/admin",       sub: user?.email || "—" },
    ...
    { label: "Sign out",          icon: "logout",   action: () => { signOut(); navigate("/login"); onClose(); },     sub: "End session" },
  ];
  return (
    <div className="popover" style={{ width: 280 }}>
      <div className="popover-head">
        <div className="sb-avatar" style={{ width: 32, height: 32, fontSize: 11 }}>{user?.initials || "·"}</div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: "var(--z-dark)" }}>{user?.name || "Not signed in"}</div>
          <div style={{ fontSize: 11, color: "var(--z-mid)" }}>{user?.email || "—"}</div>
        </div>
      </div>
      {canActAs.length > 1 ? (
        <div style={{ padding: "10px 14px", borderBottom: "1px solid var(--z-sep)" }}>
          ...
          <div className="toggle-row" style={{ width: "100%" }}>
            {[["AE","AE"],["ANALYST","Analyst"],["ADMIN","Admin"]]
              .filter(([k]) => canActAs.includes(k))   // hide entirely — not just disable
              .map(([k, l]) => (
                <button key={k}
                        className={role === k ? "on" : ""}
                        style={{ flex: 1 }}
                        onClick={() => { setRole(k); onClose(); }}>
                  {l}
                </button>
              ))}
          </div>
      ...
      <div className="client-bar">
        ...
        <div className="client-bar-r">
          <div style={{ position: "relative" }}>
            ...
            {runOpen ? (
              <div style={{ position: "absolute", top: "calc(100% + 6px)", right: 0, background: "#fff", border: "1px solid var(--z-sep)", borderRadius: 8, boxShadow: "var(--sh-lg)", padding: 6, zIndex: 100, minWidth: 320 }}>
                {entity.runs.map(r => (
                  <button key={r.id}
