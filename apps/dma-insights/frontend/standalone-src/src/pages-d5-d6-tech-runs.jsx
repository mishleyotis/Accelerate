...
/* ── Cross-pillar stories panel (Promise 10 / commit b30f9bf — standalone port)
   Fetches /api/v1/entities/:id/cross-pillar-stories on mount + on filter
   chip change. State branches:
     loading                → skeleton
     error / 404            → null (silent — never block context page)
     empty after filter     → empty state "No stories for this pillar"
     items present          → vertical list with subcap chips + why-now */
function CrossPillarStoriesPanel({ entity }) {
  const [pillarFilter, setPillarFilter] = useState("ALL");
  const [state, setState] = useState({ loading: true, items: [], narrative: null, error: null });
  useEffect(() => {
    let cancelled = false;
    if (!entity?.id || !window.DMA?.crossPillar?.storiesForEntity) {
      setState(s => ({ ...s, loading: false }));
      return;
    }
    setState(s => ({ ...s, loading: true }));
    const opts = pillarFilter !== "ALL" ? { origin_pillar: pillarFilter } : {};
    window.DMA.crossPillar.storiesForEntity(entity.id, opts).then(res => {
      if (cancelled) return;
      if (res?.ok && res.data) {
        setState({
          loading: false,
          items: res.data.stories || res.data.items || [],
          narrative: res.data.narrative_md || res.data.trend_narrative || null,
          error: null,
        });
      } else {
        setState({ loading: false, items: [], narrative: null, error: res?.error || null });
      }
    });
    return () => { cancelled = true; };
  }, [entity?.id, pillarFilter]);
  // Fail-closed: if the endpoint isn't deployed yet, render nothing.
  if (!state.loading && state.error && state.items.length === 0) return null;
  return (
    <div className="card" data-source="api" data-cross-pillar style={{ marginBottom: 14 }}>
      <div className="row" style={{ marginBottom: 12 }}>
        ...
        <div className="toggle-row">
          {["ALL", "P1", "P2", "P3", "P4"].map(p => (
            <button key={p} className={pillarFilter === p ? "on" : ""}
                    data-pillar-filter={p}
      ...
      {state.narrative ? (
        <div className="co co-teal" style={{ marginBottom: 12 }}>
          ...
          <div style={{ fontSize: 12.5, lineHeight: 1.55 }}>{state.narrative}</div>
        </div>
      ) : null}
      {state.loading ? (
        ...
        <div className="empty" style={{ padding: 24 }}>
          <div className="icon"><Icon name="sparkle" size={20} /></div>
          <h3>No cross-pillar stories</h3>
          <p>{pillarFilter === "ALL"
            ? "Stories appear once the catalogue's cross-pillar mappings land for this entity's subcaps."
            : `No stories originate from ${pillarFilter}. Try another pillar filter.`}</p>
        </div>
      ) : (
        state.items.map(s => (
          <div key={s.id || s.story_id} data-story-id={s.id || s.story_id}
               style={{ padding: "10px 0", borderBottom: "1px solid var(--z-sep)" }}>
            <div className="row" style={{ marginBottom: 6 }}>
              <span className="b b-purple">{s.origin_pillar}</span>
              <strong style={{ fontSize: 12.5 }}>{s.title || s.story_title}</strong>
              <span className="spacer" />
            ...
            {(s.sample_subcap_names || s.subcaps_touched || []).length > 0 ? (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                {(s.sample_subcap_names || s.subcaps_touched || []).slice(0, 6).map((sid, i) => (
...
function ClientContext({ entity, run }) {
  ...
  return (
    <div>
      ...
      {/* Cross-pillar stories — Analyst+ only, hidden in customer view (Promise 10) */}
      <CrossPillarStoriesPanel entity={entity} />
...
/**
 * @state-transitions
 *   ClientHealth.tab === 'patterns'
 *     → fires DMA.patterns.list(entity.subvertical) on mount/tab open
 *       - empty data → renders legacy DMA.PATTERNS table
 *       - non-empty  → renders peer_archetypes rows; the row matching
 *                      this entity's closest_archetype_label is
 *                      highlighted (teal background)
 *     The closest_archetype_label comes from /archetype (loaded
 *     separately) — when neither is loaded, no highlight.
 */
function ClientHealth({ entity, run }) {
  const { role, audience } = useApp();
  ...
  const [closestLabel, setClosestLabel] = useState(null);

  // Load archetypes for this entity's subvertical when the Patterns tab opens.
  useEffect(() => {
    if (tab !== "patterns" || !entity?.subvertical) return;
    let cancelled = false;
    window.DMA?.patterns?.list?.(entity.subvertical)?.then(res => {
      if (cancelled) return;
      if (res?.ok && Array.isArray(res.data?.items)) {
        setArchetypes(res.data.items);
      } else {
        setArchetypes([]);
      }
    });
    window.DMA?.archetype?.forEntity?.(entity.id)?.then(res => {
      if (cancelled) return;
      if (res?.ok && res.data?.closest) setClosestLabel(res.data.closest.archetype_label);
    });
    return () => { cancelled = true; };
  }, [tab, entity?.id, entity?.subvertical]);
  ...
  return (
    <div>
      <div className="page-head">
        ...
        <div className="actions">
          <button className="btn btn-tertiary"><Icon name="refresh" size={13} /> Re-run feedback file</button>
          <button className="btn btn-secondary"><Icon name="download" size={13} /> CSV export</button>
        </div>
      </div>
      ...
      {tab === "alerts" ? (
              ...
              {alerts.map(a => (
                <tr key={a.id}>
                  ...
                  <td style={{ textAlign: "right" }}>
                    <button className="btn btn-tertiary btn-sm">In review</button>
        ...
        <div className="card flush">
          <div className="card-head">
            ...
            <span className="b b-muted">{archetypes.length} archetype{archetypes.length === 1 ? "" : "s"} · {entity.subvertical || "—"}</span>
          </div>
          {archetypes.length > 0 ? (
            <table className="tbl">
              <thead><tr><th>Archetype</th><th>Subvertical</th><th>Catalogue</th><th>Defining sub-caps (top 5)</th><th>Sample</th><th style={{ textAlign: "right" }}>Silhouette</th></tr></thead>
              <tbody>
                {archetypes.map((a, i) => {
                  const isMine = closestLabel && a.archetype_label === closestLabel;
                  return (
                    <tr key={`${a.subvertical}:${a.archetype_label}:${i}`} style={isMine ? { background: "var(--z-ice)" } : undefined}>
                      <td><strong>{a.archetype_label}</strong>{isMine ? <span className="b b-teal" style={{ marginLeft: 6 }}>YOUR ARCHETYPE</span> : null}</td>
                      <td><span className="b b-purple">{a.subvertical}</span></td>
                      <td><span className="chip f-mono">{a.catalogue_version}</span></td>
                      <td>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                          {(a.defining_subcap_ids || []).slice(0, 5).map(sid => (
                            <span key={sid} className="chip f-mono" style={{ fontSize: 9.5 }}>{sid}</span>
                          ))}
                        </div>
                      </td>
                      <td>{a.sample_count}</td>
                      <td style={{ textAlign: "right" }}>
                        {a.silhouette_score != null ? (
                          <span className={`b ${a.silhouette_score >= 0.5 ? "b-teal" : "b-org"}`}>{a.silhouette_score.toFixed(2)}</span>
...
/**
 * @state-transitions
 *   ClientRuns mounts → fires DMA.entities.runHistory(entity.id)
 *     - 404                          → falls back to entity.runs (mock)
 *     - items=[]                     → "Only one run on record" empty state
 *     - len(items)>=1 + parent chain → vertical timeline newest-first;
 *                                      first item marked "ACTIVE"; older
 *                                      items marked "SUPERSEDED" with
 *                                      Δ-vs-parent score delta badge
 *   Row click                       → opens drawer with run's metadata
 *                                      (data_source, evidence_mode,
 *                                      mode_evidence_json, score summary)
 *   Row close                       → drawer state reset
 */
function ClientRuns({ entity }) {
  const [history, setHistory] = useState(null);   // { items, parent_chain }
  const [open, setOpen] = useState(null);          // selected run row

  useEffect(() => {
    let cancelled = false;
    if (!entity?.id || !window.DMA?.entities?.runHistory) return;
    window.DMA.entities.runHistory(entity.id).then(res => {
      if (cancelled) return;
  ...
  // Build "Δ vs parent" deltas: subcap_count is the closest proxy in
  // the current /run-history shape; replace with score_delta when the
  // endpoint emits it.
  const items = history?.items || [];
  const deltaFor = (cur, prevRid) => {
    if (!prevRid) return null;
    const parent = items.find(x => x.request_id === prevRid);
    if (!parent) return null;
    return (cur.subcap_count || 0) - (parent.subcap_count || 0);
  };
    ...
    <div>
      <div className="page-head">
        <div>
          ...
          <div className="sub">
            {items.length || entity.runs.length} run record{(items.length || entity.runs.length) === 1 ? "" : "s"}
            {history?.parent_chain?.length ? ` · ${history.parent_chain.length}-deep version chain` : ""}
          </div>
        </div>
        <div className="actions">
          <button className="btn btn-secondary"><Icon name="refresh" size={13} /> Trigger rerun</button>
        </div>
      </div>

      {/* Backend-driven version timeline (preferred). Falls back to the
          legacy table when the backend can't satisfy the run-history call. */}
      {items.length > 0 ? (
        items.length === 1 ? (
          <div className="card" style={{ padding: 16, fontSize: 13 }}>
            <div style={{ fontWeight: 600, marginBottom: 4 }}>Only one run on record — no version chain yet.</div>
            <div style={{ color: "var(--z-muted)" }}>request_id <span className="f-mono">{items[0].request_id}</span> · {items[0].catalogue_version} · {fmtDate(items[0].completed_at)}</div>
          </div>
        ) : (
          <div className="card flush" style={{ padding: 0 }}>
            <ul style={{ listStyle: "none", margin: 0, padding: 0 }}>
              {items.map((r, idx) => {
                const isActive = r.status === "ACTIVE" && idx === 0;
                const delta = deltaFor(r, r.parent_request_id);
                return (
                  <li key={r.request_id} style={{ display: "flex", alignItems: "stretch", borderTop: idx === 0 ? 0 : "1px solid var(--z-sep)", padding: "12px 16px", gap: 12, cursor: "pointer" }} onClick={() => setOpen(r)}>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: 18 }}>
                      <span style={{ width: 10, height: 10, borderRadius: "50%", background: isActive ? "var(--z-teal)" : "var(--z-muted)" }} />
                      {idx < items.length - 1 ? <span style={{ flex: 1, width: 2, background: "var(--z-sep)", marginTop: 2 }} /> : null}
                    </div>
                    <div style={{ flex: 1 }}>
                      <div className="row" style={{ gap: 8, marginBottom: 3 }}>
                        <span className={`b ${isActive ? "b-teal" : "b-muted"}`}>{isActive ? "ACTIVE" : "SUPERSEDED"}</span>
                        <span className="f-mono" style={{ fontSize: 11 }}>{r.request_id}</span>
                        <span style={{ color: "var(--z-muted)", fontSize: 11 }}>· {r.catalogue_version}</span>
                        {delta != null ? (
                          <span className={`b ${delta > 0 ? "b-teal" : delta < 0 ? "b-org" : "b-muted"}`} title="Δ subcap count vs parent run">
                            Δ {delta > 0 ? "+" : ""}{delta}
                          </span>
                        ) : null}
                      </div>
                      <div style={{ fontSize: 11, color: "var(--z-muted)" }}>
                        Completed {fmtDate(r.completed_at)} · {r.subcap_count} subcaps · {r.evidence_count} evidence
                        {r.parent_request_id ? <> · parent <span className="f-mono">{r.parent_request_id}</span></> : null}
                      </div>
                    </div>
                  </li>
                );
        ...
        /* Legacy fallback when /run-history is unreachable */
        <div className="card flush">
      ...
      {/* Per-row drawer */}
      {open ? (
        <aside className="ip" style={{ width: 360 }}>
          <div className="ip-head">
            <div className="ai">↻</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div className="title">Run {open.request_id}</div>
              <div className="sub">{open.status} · {open.catalogue_version}</div>
            </div>
            <button className="icon-btn" onClick={() => setOpen(null)}><Icon name="x" size={14} /></button>
          </div>
          <div className="ip-body" style={{ fontSize: 12 }}>
            <div style={{ marginBottom: 8 }}><strong>Completed</strong>: {fmtDate(open.completed_at)}</div>
            <div style={{ marginBottom: 8 }}><strong>Started</strong>: {fmtDate(open.started_at)}</div>
            <div style={{ marginBottom: 8 }}><strong>Subcaps scored</strong>: {open.subcap_count}</div>
            <div style={{ marginBottom: 8 }}><strong>Evidence rows</strong>: {open.evidence_count}</div>
            {open.parent_request_id ? <div style={{ marginBottom: 8 }}><strong>Parent</strong>: <span className="f-mono">{open.parent_request_id}</span></div> : null}
          </div>
        </aside>
      ) : null}
    </div>
...
Object.assign(window, { ClientContext, ClientHealth, ClientTechStack, ClientTechStackDetail, ClientRuns, CrossPillarStoriesPanel });
