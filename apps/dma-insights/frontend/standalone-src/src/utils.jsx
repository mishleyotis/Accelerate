...
function Icon({ name, size = 16, ...rest }) {
  ...
  switch (name) {
    ...
    // Brand-aligned feedback affordances (replace 👍/👎/💡 emoji)
    // Zennify icon system: 1.8px stroke, rounded caps, currentColor.
    case "thumb-up":   return <svg {...props}><path d="M7 22V10"/><path d="M14 2l-3 8h6.5a2 2 0 0 1 2 2.3l-1.1 7A2 2 0 0 1 16.4 21H4V10l5-8h2c1.1 0 2 .9 2 2v0z"/></svg>;
    case "thumb-down": return <svg {...props}><path d="M17 2v12"/><path d="M10 22l3-8H6.5a2 2 0 0 1-2-2.3l1.1-7A2 2 0 0 1 7.6 3H20v12l-5 8h-2c-1.1 0-2-.9-2-2v0z"/></svg>;
    case "bulb":       return <svg {...props}><path d="M9 18h6"/><path d="M10 22h4"/><path d="M12 2a7 7 0 0 0-4 12.6c.9.8 1.5 1.9 1.5 3v.4h5v-.4c0-1.1.6-2.2 1.5-3A7 7 0 0 0 12 2z"/></svg>;
...
function MaturityChip({ score, large }) {
  if (score == null || typeof score !== "number" || isNaN(score)) {
    return <span className={`b b-muted`} style={large ? { padding: "5px 9px", fontSize: 13 } : null} title="No score recorded for this run yet">Uncalc.</span>;
  }
...
/* ── Excerpt truncator (UI/UX brief S32: 240 chars + "show more") ─ */
function ExcerptText({ text, max = 240 }) {
  ...
  if (expanded) return <>
    ...
    <button onClick={(e) => { e.stopPropagation(); setExpanded(false); }}
            style={{ color: "var(--z-mid)", fontWeight: 600, fontSize: 11 }}>show less</button>
  </>;
  return <>
    {t.slice(0, max).replace(/\s+\S*$/, "")}…{" "}
    <button onClick={(e) => { e.stopPropagation(); setExpanded(true); }}
            style={{ color: "var(--z-mid)", fontWeight: 600, fontSize: 11 }}>show more</button>
  </>;
}
...
Object.assign(window, {
  ...
  AppCtx, useApp,
  Icon, BrandMark, ZennifyWordmark, PillarBadge, MaturityChip, ExcerptText, ToastStack,
