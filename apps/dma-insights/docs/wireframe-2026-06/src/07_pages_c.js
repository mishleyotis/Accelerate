      ...
      {cats.map(c => {
        ...
        return (
          <div key={c.id} className="card" style={{ marginBottom: 14 }}>
            ...
            {/* Rich subcap detail list — the grid is a summary; this is the substance (issue #4) */}
            <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 6 }}>
              <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".1em", color: "var(--z-muted)", textTransform: "uppercase", marginBottom: 2 }}>Subcap detail · click any row to drill into evidence</div>
              {subs.map(s => {
                const caps = showIssues ? DMA.issueCapsFor(s.id) : [];
                const gap = s.peerMedian - s.score;
                const evCount = DMA.EVIDENCE.filter(e => e.subcaps && e.subcaps.includes(s.id)).length;
                return (
                  <button key={`d-${s.id}`} className="subcap-row" onClick={() => onSynth(s)}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div className="row" style={{ gap: 6 }}>
                        <span style={{ fontSize: 12.5, fontWeight: 600, color: "var(--z-dark)" }} className="txt-fit-1">{s.name}</span>
                      ...
                      <div className="f-mono" style={{ fontSize: 10, color: "var(--z-muted)", marginTop: 1 }}>{s.id} · {s.confidence} confidence · {evCount} evidence item{evCount === 1 ? "" : "s"}</div>
                    </div>
                    <div style={{ width: 96, flexShrink: 0 }}>
                      ...
                      <div style={{ fontSize: 9, color: gap > 0 ? "var(--z-below)" : "var(--z-mid)", marginTop: 2, textAlign: "right" }}>{gap > 0 ? `−${gap.toFixed(1)} vs peer` : `+${Math.abs(gap).toFixed(1)} vs peer`}</div>
                    </div>
