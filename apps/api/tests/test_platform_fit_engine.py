...
def cand(name, cells=None, **kw):
    return pf.Candidate(platform=name, l3_area=kw.pop("l3_area", "Area"),
                        cells=cells if cells is not None else [cell("P1C1.1.1")],
                        **kw)
...
def test_the_same_platform_green_does_reach_it():
    ...
    assert pf.score(c)["fit_score"] >= pf.HOT_THRESHOLD
...
def test_alignment_cannot_rescue_a_platform_that_addresses_nothing():
    ...
    no gap behind it. A platform with no addressable cells says
    INSUFFICIENT_EVIDENCE whatever the objective says."""
...
def test_insufficient_evidence_is_reachable():
    ...
    row = pf.score(cand("P", [cell("P1C1.1.1", 2.0, "P1C1", ("high",), 0.0)]))
    assert row["state"] == pf.STATE_INSUFFICIENT


def test_a_well_evidenced_platform_is_ready():
    assert pf.score(cand("P", [cell("P1C1.1.1")]))["state"] == pf.STATE_READY
...
def test_scattered_cells_earn_no_interconnect():
    """A platform whose cells sit one per category lifts nothing by closing
    any of them; one that clusters does."""
    scattered = pf.score(cand("P", [cell("P1C1.1.1", cat="P1C1"),
                                    cell("P2C1.1.1", cat="P2C1"),
                                    cell("P3C1.1.1", cat="P3C1")]))
    clustered = pf.score(cand("P", [cell("P1C1.1.1", cat="P1C1"),
                                    cell("P1C1.1.2", cat="P1C1"),
                                    cell("P1C1.1.3", cat="P1C1")]))
    def inter(row):
        return next(f["value"] for f in row["factors"]
                    if f["name"] == "Catalogue interconnect")
    assert inter(scattered) == 0.0 and inter(clustered) == 1.0
    assert clustered["fit_score"] > scattered["fit_score"]
