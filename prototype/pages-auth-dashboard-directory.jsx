...
function LoginPage() {
  ...
  const signIn = async (forcedEmail) => {
    ...
    setErr(null);
    setPhase("verifying");
    // Google detects the signed-in Zennify account automatically; the typed
    // email is only an optional hint for which account to use.
    const e = ((forcedEmail || email).trim().toLowerCase()) || "mishley@zennify.com";
    await new Promise(r => setTimeout(r, 500));
    if (!e.endsWith("@zennify.com")) {
      setLoading(false);
      setPhase("idle");
      setErr("Only @zennify.com Google accounts are permitted. Please select your Zennify account.");
      return;
    }
    setPhase("granting");
    await new Promise(r => setTimeout(r, 700));
    setRole(e.includes("admin") ? "ADMIN" : e.includes("ae") ? "AE" : "ANALYST");
    setAuthed(true);
    navigate("/");
  };
  ...
  return (
    <PageShell title="Dashboard" crumbs={[{ label: "Home" }]}>
      ...
      <div className="g4" style={{ marginBottom: 14 }}>
        ...
        <KpiCard label="Avg maturity"       value={(ent.filter(e => e.overall).reduce((a, e) => a + e.overall, 0) / ent.filter(e => e.overall).length).toFixed(1)} sub={DMA.helpers.maturityLabel(ent.filter(e => e.overall).reduce((a, e) => a + e.overall, 0) / ent.filter(e => e.overall).length)} icon="heatmap"  accent="var(--z-dpur)" />
      </div>
