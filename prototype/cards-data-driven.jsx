    ...
    return (
      <div style={{ display: "grid", gridTemplateColumns: "120px 1fr 40px", gap: 8, alignItems: "center", padding: "5px 0" }}>
        ...
        <div style={{ height: 7, background: "var(--z-sep)", borderRadius: 4, overflow: "hidden" }}>
          <div style={{ width: `${pct}%`, height: "100%", background: tone, borderRadius: 4, transition: "width var(--motion-slow) var(--ease)" }} />
        </div>
        <div style={{ fontSize: 12, fontWeight: 600, color: tone, textAlign: "right", fontVariantNumeric: "tabular-nums" }}>{r.score.toFixed(1)}</div>
      </div>
    );
  };
  return (
          ...
          return (
            <div key={cat} style={{ borderBottom: "1px solid var(--z-sep)" }}>
              <button onClick={() => setOpen(o => o === cat ? null : cat)}
                ...
                <div style={{ position: "relative", height: 8, background: "var(--z-sep)", borderRadius: 4 }} title={`Band ${lo.toFixed(1)}–${hi.toFixed(1)}`}>
