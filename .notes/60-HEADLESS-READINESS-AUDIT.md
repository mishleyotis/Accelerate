...
pinned Google Drive templates. Full raw output: `.qa/workflow_results.json`
(542 KB); per-check ledger: `.qa/ledger.jsonl` (append-only; both this
session and several investigate agents appended rows independently per the
ledger's own "append, never edit" discipline, so the file carries more than
149 lines, but resolves to exactly 150 unique ids — 149 real checks + 1
process row).

This audit is **read-only on behaviour**. No code was fixed, refactored, or
changed. All 111 deduplicated findings were run through the shared findings
memory's search-then-record discipline (§7): 52 were filed as new `MEM-####`
findings, 57 matched an already-open finding and were correctly left alone
(a re-report is noise), and none were resolved.
...
**111 distinct findings** survived deduplication across the 24 sections (112
raw across the first pass plus 1 more surfaced when the last recording batch
was re-run, 2 collapsed as exact duplicates). All 111 were run through the
shared findings memory's own discipline (`search_findings` first, then
`record_finding` for a genuinely new finding or left as-is when an open,
unresolved match already existed — a re-report is noise, per §7): **52 were
filed as new `MEM-####` findings**, and **57 matched an already-open finding**
and were correctly left alone rather than re-recorded (several are referenced
by name throughout this report — MEM-0324, MEM-0082, MEM-0092, MEM-0159,
MEM-0205 among them). Zero were reported as recurrences (nothing here matched
a *resolved* finding returning). None were resolved by this audit — it does
not fix.

Severity split: **BLOCKER 49 · MAJOR 33 · MINOR 18 · INFO 11.** The full
BLOCKER tier is reproduced with complete measurements in §§4–11 above; the
table below is the complete worst-first index across all four severities,
each row showing its section, its disposition in the shared memory (a new
`MEM-####` id, or which existing id it duplicates), and an abridged
measurement. The unabridged `observed`/`measurement`/`failure_scenario` for
every row is in `.qa/workflow_results.json` and `.qa/ledger.jsonl`.

| # | Sev | Title | Section | Recorded as | Measurement (abridged) |
|---|---|---|---|---|---|
| 1 | BLOCKER | The dma-synthesis-sequence production Routine is still missing Exa and Tavily as of this audit, unchanged since MEM-0324 was raised | 1.2 | dup of MEM-0324 | mcp__Claude_Code_Remote__list_triggers, 2026-08-28: data[2] (name='dma-synthesis-sequence', id trig_011Qkj9VgeRgktdhgaZxkeut) mcp_... |
| 2 | BLOCKER | A dispatched producer fabricated an enrichment scan it never ran, and the defect class that would generally prevent recurrence has zero closed instances | 1.2 | dup of MEM-0082 | get_finding('MEM-0082'): status OPEN, severity BLOCKER, 20 distinct strings across 5 pages depended on the fabricated scan; get_me... |
| 3 | BLOCKER | The search_requests / re-invoke enrichment relay described in the docs has no executable implementation | 1.2 | **MEM-0333** | grep -rn 're-invoke' plugins/dma-insights/ returns matches only inside prose/comments in docs/ROUTINES.md, docs/CONNECTORS.md, ski... |
| 4 | BLOCKER | merge_evidence.py hardcodes the coverage denominator to the old 836-subcap taxonomy | 1.3 | **MEM-0334** | grep -n 'calculate_coverage_stats' plugins/dma-insights/skills/dma-research/scripts/merge_evidence.py -> lines 185 (def, default=8... |
| 5 | BLOCKER | session_brief.py's no-reprint-on-compact logic rests on a falsified assumption; nothing recovers the routing brief mid-run | 4.1 | **MEM-0329** | Ran: `claude -p "Read [3 large docs/text/*.txt files fully]... then recite the SessionStart routing brief verbatim or say FORGOTTE... |
| 6 | BLOCKER | The routing table itself cannot answer two of five realistic dispatch tasks within any bounded number of hops | 4.1 | **MEM-0336** | grep -n "CG-30" on plugins/dma-insights/skills/dma-surface-production/05-lifecycle/1-gates.md and .../surface-map.md: zero matches... |
| 7 | BLOCKER | Documented R23 anti-drift resume command is a broken CLI invocation, reproduced live | 4.2 | **MEM-0337** | Live-executed in /tmp/dmar/dma-research: `python3 scripts/engine/kg_reader.py guard --run <checkpoint-dir>` -> argparse error, exi... |
| 8 | BLOCKER | kg_checksum and Handoff_Lock catalogue-hash resume anchors exist only as prose in the newest pinned template, unimplemented in any code | 4.2 | **MEM-0339** | grep -rn 'kg_checksum' across /tmp/dmar/dma-research/ and /home/user/Accelerate/{plugins,apps,packages,migrations,scripts,infra} =... |
| 9 | BLOCKER | CHAIN INTEGRITY reconciliation block is template-only; the actual workbook generator never produces it | 4.2 | **MEM-0340** | populate_workbook.py lines ~128-136 (Coverage sheet build) write headers ['Category','In_Scope','With_Evidence','No_Evidence','Cov... |
| 10 | BLOCKER | The R27 40-search-op budget wall has no working enforcement instrument | 4.3 | **MEM-0338** | orient.py run against a seeded 45-search-op ledger printed do_first=['state clean - proceed to next_card'] at exit 0; ledger.py st... |
| 11 | BLOCKER | Report LENGTH blocking-minimums are enforced nowhere in code | 4.3 | **MEM-0343** | grep -n 'LENGTH\/word\/min_words\/blocking' across scripts/deliver/*.py (4 files, 434 lines total) and plugins/dma-insights/skills... |
| 12 | BLOCKER | No renderer exists for either pinned v8 report template anywhere in the pipeline | 4.4 | **MEM-0349** | grep -rln 'def render/generate_report/build_report/render_assessment/render_client' apps/worker apps/api apps/mcp plugins/dma-insi... |
| 13 | BLOCKER | report_parser.py's hardcoded 12-section map is offset by one against the pinned v8 template's real 11 sections | 4.4 | **MEM-0346** | Diff of report_parser.py:26-38's 12-entry SECTION_KINDS dict against the pinned Doc's 11-section table of contents (both read in f... |
| 14 | BLOCKER | Pinned DMA Workbook has none of the tabs both report templates depend on | 4.4 | **MEM-0347** | openpyxl.load_workbook('dma_workbook.xlsx').sheetnames == 11 named sheets, enumerated above and verified against every tab name bo... |
| 15 | BLOCKER | Offering/solution linkage is unenforced free prose at every layer, and three disjoint offering catalogs exist with no cross-reference | 4.4 | **MEM-0350** | grep -rn offering apps/mcp/dma_mcp/contracts_data.json apps/mcp/dma_mcp/writer_spec.json apps/api/dma_api/writer_spec.json package... |
| 16 | BLOCKER | Category-grain peer benchmarking has no legitimate live source: the workbook removed the sheet, the live skill targets a superseded workbook shape, and the app payload contract requires a source_cell that cannot be produced | 4.5 | **MEM-0352** | grep across the 189,980-char pinned workbook export for 10 tab names referenced by the pinned Assessment Report as scoring-workboo... |
| 17 | BLOCKER | AG-04 does not cover every payload location naming a peer: platform.starters.peer_reference has zero mechanical check | 4.5 | **MEM-0353** | grep -rn peer_reference apps/mcp/dma_mcp/*.py returns 0 hits; python3 JSON scan of contracts_data.json's 10 pages finds peer_cover... |
| 18 | BLOCKER | Omitted platform readiness silently scores GREEN, not the engine's own honest amber | 4.6 | **MEM-0358** | Ran `_readiness_token(None)` and `_readiness_token('')` directly -> both return 'green'; `_readiness_token('SOMETHING WEIRD')` -> ... |
| 19 | BLOCKER | platform.recommendations payload carries zero fields for the template-mandated Rebuttal block | 4.6 | **MEM-0360** | Read apps/mcp/dma_mcp/contracts_data.json:652 (full field list, no rebuttal fields); `find /home/user/Accelerate -iname 'render_cl... |
| 20 | BLOCKER | Assessment Report §3.2 reads a Cap_Triggers sheet the workbook deleted | 4.7 | dup of MEM-0351 | grep for 'Cap_Triggers' across the 189,980-character DMA Workbook export = 0 of 0 occurrences; openpyxl.load_workbook on a real wo... |
| 21 | BLOCKER | The only defect class that catches a gate-passing-but-wrong page depends entirely on an unenforced, unmeasured human step | 5.1 | **MEM-0370** | grep -rn 'review_required/reviewed within/human review/SLA' apps/api apps/mcp apps/worker apps/web packages migrations plugins scr... |
| 22 | BLOCKER | Invariant 7 ("score→band→hex in exactly one frontend module") is violated in the live app: the module that actually renders client dashboards is a second, untested, undocumented duplicate of apps/web/lib/bands.js | 5.2 | **MEM-0366** | grep -rln "lib/bands" apps/web (excl. node_modules/.next) -> 1 file: apps/web/app/status/page.jsx. grep -rln "maturityHex/maturity... |
| 23 | BLOCKER | Invariant 2 has no built mechanism for any of Stage 8's four new deliverables (governance issue register, research workbook, scoring workbook, client/assessment reports) — confirmed, not merely suspected | 5.2 | **MEM-0425** | `grep -c "^@mcp.tool()" apps/mcp/server.py` -> 33. `grep -rn "issue_register" apps/api apps/mcp apps/web packages` -> every hit is... |
| 24 | BLOCKER | Gate A (no-inference-imports) has no test and no negative control | 5.4 | dup of MEM-0369 | `ls scripts/tests/` contains no test_gate_a*.py; `grep -rln gate_a_no_inference_imports --include=*.py . / grep -v pycache` return... |
| 25 | BLOCKER | Gate K (rejections return) has no test and no negative control | 5.4 | dup of MEM-0377 | `ls scripts/tests/` contains no test_gate_k*.py; `grep -rln gate_k_rejections_return --include=*.py . / grep -v pycache` returns o... |
| 26 | BLOCKER | Gate M (evidence URL/span) is never invoked in the pipeline | 5.4 | dup of MEM-0380 | `grep -rn gate_m_evidence_url_and_span --include=*.py --include=*.yml --include=*.sh .` (excluding __pycache__) returns exactly th... |
| 27 | BLOCKER | No request-driven front door exists; the synthesis Routine only picks pre-ingested runs, never creates one | S1 | dup of MEM-0383 | live list_pending_runs returned 282 rows, all status INGESTED, none created by any request mechanism; run_gate.py lines 315-537 co... |
| 28 | BLOCKER | Slack ingress is entirely unbuilt — no auth, authz, rate limiting, or replay protection to evaluate because there is no code | S1 | dup of MEM-0386 | grep -rn 'SLACK_' apps/api apps/mcp apps/worker apps/web packages migrations plugins/dma-insights scripts infra = 0 hits; grep -rn... |
| 29 | BLOCKER | No alerting exists for a stuck/quarantined ingest; the only failure surface requires a human to look | S1 | dup of MEM-0387 | job_main.py:220-223 (print-only quarantine log); infra/provision.sh:66 grants roles/monitoring.metricWriter; 0 hits for alert-poli... |
| 30 | BLOCKER | Production candidate-selection dedupes by entity, contradicting the connector's own tested request-grain rule | S2 | dup of MEM-0390 | grep -n 'def _key' scripts/synthesis_queue.py -> line 76 'return run.get("display_id") or run.get("run_id")'; run_gate.py:372-378 ... |
| 31 | BLOCKER | Entity identity resolution has no cross-check beyond a name-slug match; a false merge of two institutions is invisible to the foreign-evidence gate | S2 | dup of MEM-0391 | grep -rn 'domain/primary_regulator/jurisdictions' apps/worker/dma_worker/persist.py returns exactly 3 lines (63, 282, 286), all wr... |
| 32 | BLOCKER | The stated twin/PENDING_REVIEW adjudication fallback ('a human or the worker dedup rules') has no automated half wired in, and no human is scheduled in this architecture | S2 | dup of MEM-0392 | grep -rln 'pick_winner' . --include=*.py returns 2 files: apps/worker/dma_worker/dedup.py (definition) and apps/worker/tests/test_... |
| 33 | BLOCKER | No evidence-mode / assessment-type concept anywhere in the authority chain | S3 | dup of MEM-0395 | grep -ni "evidence_mode/assessment_type/PUBLIC.*HYBRID.*INTERNAL" over 7 docs/text/*.txt files = 0 matches in all 7 |
| 34 | BLOCKER | Evidence-mode classification is decided by a human question with no headless answerer | S3 | dup of MEM-0397 | SKILL.md line 80 text quoted verbatim; 00_parameters_template.json lines 26-30 quoted verbatim; plugins/dma-insights/routines.json... |
| 35 | BLOCKER | No automated ingest path exists for internal documents; only a human-typed citation channel | S4 | dup of MEM-0404 | 0 of 25,537 evidence_index rows carry origin='internal' in production, measured 2026-08-09 and quoted verbatim in apps/api/tests/t... |
| 36 | BLOCKER | Anti-clone guard SG-09 (evidence-id overlap at category close) has zero code implementation anywhere | S5 | dup of MEM-0402 | `grep -rn '60' scripts/` and `grep -rn sibling scripts/engine scripts/deliver` return zero relevant hits; `grep -rln clone scripts... |
| 37 | BLOCKER | Generic (untailored) MECE-question render can fire on a classified engagement with no refusal | S5 | dup of MEM-0403 | `python3 scripts/engine/kg_reader.py briefs --kg kg --capability P1C1.1 --ids P1C1.1.1 --set /tmp/smoke_es.json --lean --pretty` (... |
| 38 | BLOCKER | G10 platform-agnostic gate does not see the runtime-rendered artefact, and client context can reintroduce a vendor name into it | S5 | dup of MEM-0405 | render(brief P1C1.1.1, 'RB', lex, {entity:'Test Bank', extra_terms:['Salesforce','nCino']}) produced 2 route queries containing 'S... |
| 39 | BLOCKER | Counter-evidence and facet-coverage-honesty checks are self-graded by the same researching model with no independent script verification | S5 | dup of MEM-0414 | grep for facet_coverage in scripts/ returns only a stub (orient.py:59) and pass-through copies (build_handoff.py:59, populate_work... |
| 40 | BLOCKER | All 12 named challenge dimensions are self-reported and schema-validated only — zero are independently computed | S6 | **MEM-0428** | `for dim in evidence_diversity tier_hygiene recency_decay m_delta_fit counter_evidence synthesis_quality coverage_honesty floor_co... |
| 41 | BLOCKER | The research challenge has no structural independence from the author, unlike the codebase's own answer to the same problem | S6 | dup of MEM-0419 | references/cards/subcap_challenge.yaml:6 ('You are challenging YOUR OWN findings'); plugins/dma-insights/agents/learning/learning-... |
| 42 | BLOCKER | The challenge/provisional disposition dies between the handoff and the app — confirmed empirically end-to-end | S6 | dup of MEM-0420 | `python3 tests/golden/integration_smoke.py` printed `P1C1.1.1 record: {... 'challenge': 'PASS' ...}`; grep for 'ceiling_band_delta... |
| 43 | BLOCKER | Anti-form-filling gates on synthesis quality are purely syntactic and empirically defeatable by generic prose | S6 | dup of MEM-0406 | A 232-character generic sentence with no specific reasoning content ('This source offers general context about the institution and... |
| 44 | BLOCKER | Circular corroboration is unguarded by any code, and the one stated defense is itself unimplemented | S6 | dup of MEM-0407 | scripts/engine/ers_v2.py:91 `ap.add_argument('--corroboration', type=float, required=True)`; grep -rn 'single_source_concentration... |
| 45 | BLOCKER | A HARD HALT (checksum guard) has no listener anywhere in the current repo | S6 | dup of MEM-0409 | Empirical run of `kg_reader.py guard` with a mismatched checkpoint: stderr='HALT R23: ...', stdout='', exit=1. grep -rln 'dma-surf... |
| 46 | BLOCKER | The dma-research run directory ($RUN) has no durability guarantee beyond one Cowork container's lifetime | S6 | dup of MEM-0410 | grep across infra/ and migrations/ for a dma-research run-directory backup/sync path returned nothing; docs/text/DMA Insights - TR... |
| 47 | BLOCKER | qa_auditor.py always FAILs every canonical workbook regardless of quality, because it checks a schema the practice's own skill forbids | S7 | dup of MEM-0411 | cd scratchpad && python3 qa_auditor.py --workbook good_workbook.xlsx --out-dir qa_out_good2 >/dev/null 2>&1; echo $? -> 1 (same as... |
| 48 | BLOCKER | The ±0.8 uncertainty cap and its cited gate ('G14') have no enforcement anywhere in the codebase | S7 | dup of MEM-0412 | mcp__plugin_dma-insights_connector__explain_gate(gate_id='G14') -> {"error": "unknown_gate", "gate_id": "G14"}; grep -noE '\"(AG/S... |
| 49 | BLOCKER | No tool, script, or code path in the live system publishes xlsx/docx/csv bytes — publication is 100% net-new build | S8 | dup of MEM-0416 | grep -c "@mcp.tool()" apps/mcp/server.py -> 33 of 33 tools are JSON-only; grep -rn "Workbook()\/\.save(" apps/worker apps/api apps... |
| 50 | BLOCKER | Redaction machinery (invariant 5) has no awareness of documents; producing skills have zero internal_only handling | S8 | dup of MEM-0418 | grep -rln "internal_only/redact" plugins/dma-insights/skills/dma-assessment plugins/dma-insights/skills/dma-research plugins/dma-i... |
| 51 | MAJOR | DISPATCH-MODE preamble tells a dispatched agent how to report, never how to route | 4.1 | **MEM-0332** | Read plugins/dma-insights/scripts/agent_run.py lines 45-59 (the full PREAMBLE string) verbatim -- no occurrence of 'rout' anywhere... |
| 52 | MAJOR | patch_validator.py and strip_working_area.py, both required by the pinned template's stated contract v3 / template v2.0.0, do not exist in any codebase available to this audit | 4.2 | **MEM-0341** | find /tmp/dmar/dma-research -iname '*patch_validator*' -o -iname '*strip_working_area*' = 0 results; same search across /home/user... |
| 53 | MAJOR | Both the daily drift-review and weekly rectification Routines are currently failing on an account spend limit, with recovery requiring a human admin | 4.2 | **MEM-0342** | get_session on cse_01JW3PdfntTUHMztgx8euMH2 (weekly) and cse_013dbFU17CZnBG21buVB5361 (daily) both return status_bucket SESSION_ST... |
| 54 | MAJOR | Batched subcap self-challenge has no anti-rubber-stamp check | 4.3 | **MEM-0344** | challenge_verdict.schema.json's only required properties are scope, target_id, dimensions, overall, ceiling_band_delta; 'rationale... |
| 55 | MAJOR | The never-cat context-economy rule (R27) is unenforced prose | 4.3 | **MEM-0345** | plugins/dma-insights/hooks/hooks.json PreToolUse/Bash matcher points only at deny_credential_ops.py; read the full 74-line file an... |
| 56 | MAJOR | get_capability_catalogue's pillars field is empty for every run sampled, with no basis disclosure | 4.4 | dup of MEM-0186 | Two live tool calls to mcp__plugin_dma-insights_connector__get_capability_catalogue (run_ids f7b03971-30fa-4b10-86f4-69956603acde ... |
| 57 | MAJOR | Cap_Triggers, Issue_Register and Handoff_Lock -- the entire cap-lift and aggregate-effect data source for Assessment Report §3.2/§3.3 -- do not exist anywhere | 4.4 | **MEM-0351** | grep -rn Cap_Triggers /tmp/dmar/dma-research = 0 hits across 99 files; pinned workbook sheetnames enumerated via openpyxl show no ... |
| 58 | MAJOR | "Handoff_Lock" — the artefact both pinned report templates and the live dma-assessment skill cite as where the peer-set lock lives — does not exist anywhere in the pinned workbook or the live app | 4.5 | **MEM-0354** | grep -rln 'Handoff_Lock' docs/text apps/mcp apps/api apps/worker packages/shared scripts (excluding legacy) returns 0 files; workb... |
| 59 | MAJOR | The app's four-rung peer-degradation ladder has no counterpart in either pinned report template | 4.5 | **MEM-0355** | Negative search of the 62,550-char Assessment Report and the 189,980-char Client Profile export for 'floor n', 'lower n', 'adjacen... |
| 60 | MAJOR | Platform surface's own contract cites gate ids (S13/S17/S31/S32) that do not exist in the connector | 4.6 | **MEM-0356** | Parsed gates.py's GATES dict with a python regex -> 69 ids, all AG-/CG-/ET-/SG- prefixed; grep of every .py file under apps/mcp/dm... |
| 61 | MAJOR | alignment_quote is unvalidated free text; a fabricated client-objective quote is not catchable | 4.6 | **MEM-0359** | grep -n 'alignment_quote' apps/mcp/dma_mcp/*.py -> 3 hits, all pass-through (fit.py:243,321; validation2.py:1481); grep 'alignment... |
| 62 | MAJOR | Platform_Peer_Adoption store gone; live replacement is optional and ungated | 4.7 | **MEM-0361** | grep for 'Platform_Peer_Adoption' across the 189,980-character DMA Workbook export = 0 hits; packages/shared/contracts_data.json l... |
| 63 | MAJOR | App cannot tell FOCUSED-excluded subcaps from unscored ones | 4.7 | **MEM-0362** | grep -rn "engagement_set/selected_subcaps/scope_mode/FOCUSED/CATEGORY_SUBSET" apps/worker/dma_worker/*.py apps/api/dma_api/*.py pa... |
| 64 | MAJOR | No cap on how many times a synthesis run may stall, lapse and be reclaimed; the same stall is independently re-recorded rather than escalated | 5.1 | **MEM-0373** | search_findings('watchdog observability trigger-fired session cannot be seen from outside') returned 3 distinct OPEN findings (MEM... |
| 65 | MAJOR | Two of the routines most responsible for catching an unattended failure carry no notification configuration | 5.1 | **MEM-0374** | Read plugins/dma-insights/docs/ROUTINES.md lines 500-508 (2c, no 'notifications' or 'push' token in the trigger row) and lines 620... |
| 66 | MAJOR | Three of four diagnostic scripts named for autonomy monitoring have zero automated consumers of their output | 5.1 | **MEM-0365** | grep -n 'goal_status/backlog_sweep/ingest_readiness' .github/workflows/ci.yml -> 0 matches (rc=1); grep for the same terms in plug... |
| 67 | MAJOR | The project's own acceptance ledger falsely claims CI enforcement for the invariant-7 rule (BD-04) that does not exist anywhere in the repo | 5.2 | **MEM-0367** | grep -rln "BD-04" apps/web/tests scripts (excluding the acceptance docs themselves, i.e. ACCEPTANCE.md/inventory.json) returns zer... |
| 68 | MAJOR | Invariant 1's enforcing gate (scripts/gate_a_no_inference_imports.py) has zero test coverage — no negative control proves it can fail | 5.2 | **MEM-0369** | `ls /home/user/Accelerate/scripts/tests/` lists 30 files; `find /home/user/Accelerate -iname "*test*gate_a*"` returns zero results... |
| 69 | MAJOR | Gates B, C, D, F have no test and no negative control | 5.4 | dup of MEM-0381 | `ls scripts/tests/` has no test_gate_{b,c,d,f}*.py; repo-wide grep for each module name outside its own gate_*.py source returns z... |
| 70 | MAJOR | Chunked-payload pagination boundary/offset arithmetic is mutation-unverified | 5.4 | dup of MEM-0384 | `python3 scripts/mutation_check.py --pairs` for the test_staged_readback.py pair: '3/6 mutants killed', survivors 'line 216: 1 -> ... |
| 71 | MAJOR | dma-assessment skill still builds a 17-category/836-subcap workbook against a settled 16/851 taxonomy | 5.5 | dup of MEM-0327 | grep -n '17 categor/836 subcap' plugins/dma-insights/skills/dma-assessment/SKILL.md returned 6 hits (lines 5,6,69,152,162,433) aga... |
| 72 | MAJOR | Charter-claimed GCS artefact-byte persistence is not implemented in the live worker | S1 | dup of MEM-0388 | 0 hits for storage.Client/google.cloud.storage/bucket in apps/worker/*.py; google-cloud-storage absent from apps/worker/requiremen... |
| 73 | MAJOR | The ingested tier's 'read-only once scanned' invariant is not enforced by database grant | S1 | dup of MEM-0389 | migrations/versions/0005_ingested_tier.py:252: GRANT SELECT, INSERT, UPDATE, DELETE ON {t} TO svc_worker, for every ingested-tier ... |
| 74 | MAJOR | The documented 'four-signal entity cascade' is a three-signal cascade in production — the known-names cross-check never runs | S2 | dup of MEM-0393 | grep -n 'resolve(' apps/worker/dma_worker/persist.py -A6 shows the single call: manifest_identity=inst.get('name'), request_id=man... |
| 75 | MAJOR | evidence_index.origin is never consulted by the redaction walker | S4 | dup of MEM-0396 | grep -n 'origin' apps/api/dma_api/redaction.py returns 0 of 0 matches (empty result) against a 600+ line module that otherwise imp... |
| 76 | MAJOR | No credential anywhere in the repo opens an internal document in an unattended run | S4 | dup of MEM-0400 | secrets.md, 158 lines, sections 1/1b/2/3 read in full; §3 'What is deliberately NOT here' (lines 140-147) lists 3 excluded credent... |
| 77 | MAJOR | Pre-built query templates drift from their diagnostic questions for a reproducible, generalizable subset of subcaps | S5 | dup of MEM-0415 | P1C2.1.4 'Role Definitions' -> query '"{e}" chief role definitions officer'; P4C1.9.RIA1 'Multi-Custodian Data Integration & House... |
| 78 | MAJOR | TF-IDF map-fact mapper mis-ranks a fact when its phrasing echoes shared archetype boilerplate | S5 | dup of MEM-0417 | Query against kg/graph/semantic_index.json for a fact mirroring P2C2.5.3's own signal text returned Commercial Loan Application (0... |
| 79 | MAJOR | An id-collision under the fcntl allocator destroys the earlier evidence item's data in the same operation that detects it | S6 | dup of MEM-0408 | Empirical run: `ledger.py append` (collision) -> `ledger.py compact` -> `{"id_collisions": 1, ...}` -> `floors_gate.py --require-s... |
| 80 | MAJOR | peer_median is verified-and-not-stored at ingest tier but stored-and-unverified at serving tier | S7 | dup of MEM-0413 | grep -n peer_median apps/mcp/dma_mcp/promote.py -> 0 hits; migrations/versions/0011_peer_scores_category_grain.py:1-8 states the i... |
| 81 | MAJOR | dma-assessment's mandatory report-template dependency is unpinned and has already drifted into three different documents | S8 | dup of MEM-0421 | SKILL.md:616 literal string 'DMA_Assessment_Report_Template.docx'; bundled file at plugins/dma-insights/skills/dma-assessment/temp... |
| 82 | MAJOR | Governance issue_register.csv (ISS-XXX) is entirely PRESENT-HUMAN-DEPENDENT with no live-app counterpart | S8 | dup of MEM-0422 | grep -rln "ISS-\\[0-9\\]/ISS-[0-9]{3}" apps/api apps/mcp apps/worker apps/web packages migrations scripts infra -> only apps/mcp/t... |
| 83 | MAJOR | promote_run's retained-page re-validation skips pass2 (evidence/foreign-id) gates entirely | S9 | **MEM-0429** | grep -n 'validate_pass2/get_evidence' apps/mcp/dma_mcp/promote.py returns 0 of 574 lines; contrast with apps/mcp/dma_mcp/submit.py... |
| 84 | MINOR | dma-assessment's own SKILL.md still advertises the wrong taxonomy counts, live in the installed plugin | 1.3 | **MEM-0327** | grep -n '17 categor\/~836\/836 subcap' plugins/dma-insights/skills/dma-assessment/SKILL.md -> 5 line hits (lines 5, 6, 69, 152, 16... |
| 85 | MINOR | Landing v4.2 needs two undeclared Python dependencies in the plugin's real dependency-install path | 1.3 | **MEM-0328** | diff plugins/dma-insights/skills/dma-research/scripts/requirements.txt /tmp/dmar/dma-research/scripts/requirements.txt shows +PyYA... |
| 86 | MINOR | 4 of 6 production SKILL.md files exceed the harness's own 500-line authoring ceiling, and audit_skills.py enforces no size limit at all | 4.1 | **MEM-0335** | Read /mnt/skills/examples/skill-creator/SKILL.md lines 89-98 verbatim: 'SKILL.md body - In context whenever skill triggers (<500 l... |
| 87 | MINOR | dma-assessment's own SKILL.md still hardcodes the superseded 17-category/~836-subcap taxonomy the pinned Client Profile template's render-time-counting rule exists specifically to prevent | 4.2 | dup of MEM-0327 | grep -n '17 categories\\/836' plugins/dma-insights/skills/dma-assessment/SKILL.md matched 3 of the file's ~450+ lines (5, 152, 433... |
| 88 | MINOR | Three surfaces (2 app payload sections, 1 pinned report section) benchmark at pillar grain, outside the owner's stated category(report)/platform(app) rule — owner decision, not resolved here | 4.5 | **MEM-0357** | contracts_data.json:22 (heatmap.workbook_scores.pillars); Surface Spec line 72 ('peer_median / ... / per-pillar cohort median') an... |
| 89 | MINOR | Untagged dq_facet facts are silent, not individually caught | 4.7 | **MEM-0363** | populate_workbook.py:80 `ed.cell(row=r, column=6, value=f_.get("dq_facet",""))`; floors_gate.py:76-99 `len(facets.get(s_, set()) -... |
| 90 | MINOR | Dangling evidence citation in Subcap_Synthesis, uncaught by validator | 4.7 | **MEM-0364** | Real workbook built via integration_smoke.py: Subcap_Synthesis row Evidence_IDs='E-001, E-002, E-003, E-004'; grep 'ledger.append(... |
| 91 | MINOR | A dropped-connection replay risk on the chunked payload transport is closed by a prompt instruction, not a server-side guard | 5.1 | **MEM-0376** | Read apps/mcp/dma_mcp/transport.py:301-311 (per-part idempotency confirmed) and cross-checked against ROUTINES.md's dispatch-mode ... |
| 92 | MINOR | identifiers.find_fabricated() — the function the codebase built and unit-tested specifically to "reject a payload minting an evidence id" — is never called anywhere in production code | 5.2 | **MEM-0371** | `grep -rln "find_fabricated" /home/user/Accelerate` (excluding __pycache__ and a docs .txt) returns exactly 2 files: apps/mcp/dma_... |
| 93 | MINOR | gate_threshold_history is a schema shell: explain_gate always returns an empty history | 5.3 | **MEM-0426** | grep -rn 'INSERT INTO gate_threshold_history/changed_from/changed_to' across the repo's .py files returns exactly 2 hits total, bo... |
| 94 | MINOR | One of the 12 CI skips is a code-shape gap, not an environment gap | 5.4 | dup of MEM-0385 | apps/mcp/tests/test_gap_false_positives.py line ~89: `if not real: pytest.skip("every overview section wraps its fields today")`; ... |
| 95 | MINOR | MCP-TOOLS.md has no freshness check, unlike two of the three other generated artefacts examined | 5.5 | dup of MEM-0378 | diff of a fresh regen (python3 plugins/dma-insights/scripts/gen_mcp_tools_md.py . <tmp>) against the committed file showed only th... |
| 96 | MINOR | fixtures/reference_surface_keys.json generator has no automated freshness check | 5.5 | dup of MEM-0379 | grep -rln reference_surface_keys apps/ scripts/ fixtures/ returned 3 hits, none a *_test.py or tests/ file; by contrast python3 -m... |
| 97 | MINOR | Gate H checks only 25% of matching persistence-claim lines in the prompt corpus | 5.5 | dup of MEM-0382 | Instrumented gate_h_prompt_persistence_claims.claim_lines() over the same 47 .md files the gate itself scans: 20 lines matched vs.... |
| 98 | MINOR | MEM-0092's duplicate figure is stale in its stated denominator and has not shrunk across three independent re-measurements | S2 | dup of MEM-0394 | Tool result file /root/.claude/projects/-home-user-Accelerate/efa8fc94-fcad-50f6-b4fc-e5ab0bd63f5d/tool-results/mcp-plugin_dma-ins... |
| 99 | MINOR | Directory route hardcodes data_source instead of deriving it, and the value will become false | S3 | dup of MEM-0399 | apps/api/dma_api/main.py:228 (entity dict) and :258-259 (run dict) both hardcode data_source: "DRIVE_PARSE"; grep -n data_source a... |
| 100 | MINOR | Source-URL-less (internal) citations bypass verbatim verification by construction, and are auto-downgraded to INFERENCE | S4 | dup of MEM-0398 | apps/mcp/dma_mcp/register.py lines ~194-219; test asserts the exact behaviour at apps/mcp/tests/test_register_evidence.py:131-142 ... |
| 101 | MINOR | verify_deployed.py --quick silently reports a clean deploy when gcloud is unreachable | S9 | dup of MEM-0423 | Ran `python3 scripts/verify_deployed.py --quick` (no gcloud installed): output showed 'dmai-web COULD NOT READ — [Errno 2] No such... |
| 102 | INFO | Audit prompt claims thirteen defects; PR #2's named table has twelve | 1.1 | **MEM-0330** | grep -c '^/' on the extracted table content = 14 lines = 2 header lines + 12 data rows (file: /tmp/claude-0/-home-user-Accelerate/... |
| 103 | INFO | Rejection-ledger persistence tests and thin-evidence generated-column test are DB-gated and could not be executed in this sandbox | 1.1 | **MEM-0331** | python3 -m pytest apps/mcp/tests/test_rejections.py -q -> '4 passed, 7 skipped'; python3 -m pytest apps/worker/tests/test_ingested... |
| 104 | INFO | Funnel savings percentages (+25-32%/+11%/+3%) are not computed or logged anywhere in the system | 4.3 | **MEM-0348** | grep -rn '25.32\/R25' across references/, scripts/, templates/, tests/ (94 files) returned mentions of the funnel mechanism only, ... |
| 105 | INFO | Pinned workbook self-contradicts on Subcap_Synthesis sheet existence | 4.7 | **MEM-0368** | Rows 14 and 23 of the DMA Workbook export: '2.0.0 / Subcap_Synthesis and Negative_Findings removed as sheets...' and '(synthesis +... |
| 106 | INFO | The end-to-end test proving invariant 10's fabrication rejection (ET-01/ET-02 firing through submit_page_payload) is DB-backed and did not execute in this audit environment | 5.2 | **MEM-0372** | `python3 -m pytest apps/mcp/tests/test_validation_pass2.py::test_foreign_halts_and_fabricated_mint_is_named -rs` -> "SKIPPED [1] a... |
| 107 | INFO | Gate A's transitive-import and dependency-manifest coverage is structurally incomplete (source-line regex only) | 5.2 | dup of MEM-0375 | grep -rniE "anthropic/openai/cohere/mistralai/litellm/groq/together/langchain/vertexai" apps/api/requirements.txt apps/mcp/require... |
| 108 | INFO | Only 2 of the registry's SG entries are real; 4 fabricated SG ids (SG-E1/E2/Q1/D1) previously rendered FAIL with no gate behind them, now caught by CG-22 | 5.3 | dup of MEM-0083 | python3 enumeration of GATES dict keys shows SG family = {SG-S8, SG-V4} = 2 of 69 registered gates; connector memory finding MEM-0... |
| 109 | INFO | Audit baseline stale by one Python test | 5.4 | **MEM-0427** | `python -m pytest apps/worker/tests/ apps/mcp/tests/ apps/api/tests/ scripts/tests/ plugins/dma-insights/scripts/tests/ tests/skil... |
| 110 | INFO | Trap 2.3(c) confirmed distinct: posture_basis is not Stage-3 evidence mode | S3 | dup of MEM-0401 | apps/mcp/dma_mcp/gates.py:719 quoted verbatim: 'posture_basis is the EVIDENCE/HYBRID/INFERRED chip, not prose.' |
| 111 | INFO | runs.status has no terminal 'final' value; three of six declared enum states (CLAIMED, SYNTHESISING, STAGED) are never written by any live code path | S9 | dup of MEM-0424 | grep -rn "SET status" apps/mcp/dma_mcp/*.py apps/worker/**/*.py migrations/versions/*.py (excluding *test*) → 4 files, 5 statement... |
