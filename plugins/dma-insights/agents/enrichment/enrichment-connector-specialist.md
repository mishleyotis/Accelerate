...
|---|---|---|---|---|
| `techstack` | `techstack.techstack` | `explorium` — the scan at ingest | T1 | **wired, not live** |
...
- **A credential that does not exist yet.** Explorium is `wired, not live`:
  *"no live API key exists in Secret Manager, the routine records NOT_RUN with
  that reason"*. `NOT_RUN` **with the reason** is the recorded state. Never
  substitute a different scan and call it the Explorium pass.
