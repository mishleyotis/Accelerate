...
The research stage is closed: every category in P3 passed its floors gate
...
Work one capability (P3Cx.y) at a time so its subcaps DIFFERENTIATE — the
...
```
python3 -m engine.assessment score --run <R> --root <ROOT> --subcap P3C1.1.1 \
    --score 2.5 --confidence MEDIUM --actor scoring-p3-producer \
...
Every P3 row carries a score (`engine.assessment state` shows
...
records its pass on P3; you never record it yourself, and you never run
