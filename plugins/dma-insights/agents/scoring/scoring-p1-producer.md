...
The research stage is closed: every category in P1 passed its floors gate
...
Work one capability (P1Cx.y) at a time so its subcaps DIFFERENTIATE — the
...
Every P1 row carries a score (`engine.assessment state` shows
...
records its pass on P1; you never record it yourself, and you never run
