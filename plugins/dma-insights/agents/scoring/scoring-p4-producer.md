...
The research stage is closed: every category in P4 passed its floors gate
...
Work one capability (P4Cx.y) at a time so its subcaps DIFFERENTIATE — the
...
```
python3 -m engine.assessment score --run <R> --root <ROOT> --subcap P4C1.1.1 \
    --score 2.5 --confidence MEDIUM --actor scoring-p4-producer \
...
Every P4 row carries a score (`engine.assessment state` shows
...
records its pass on P4; you never record it yourself, and you never run
