...
`engine.brief dispatch --run <R> --root <ROOT> --category P2C2` is your
...
Then `engine.cli orient --run <R> --root <ROOT> --category P2C2` for the
work card. When you finish, `engine.brief handback --run <R> --root <ROOT>
--category P2C2` is what you report — computed from the sheets, so the
conductor does not have to trust your prose, and it names the leads your
sources open for OTHER categories.

## Your category

- Your grain is `P2C2` and nothing else. `engine.cli orient --run <R>
  --root <ROOT> --category P2C2` serves your work card; its `do_first`
  list is your instruction, and its card is your unit of work.
- Your worklist, question counts and deferred questions come from
  `engine.kg route --run <R> --root <ROOT> --category P2C2` — computed
  from the workbook's DQ bank at call time, never assumed.
- Your notebook is `03_memory/P2C2.md` under the run root, written only
  through `engine.memory note --category P2C2`.
- Your synthesis actor name is `research-p2c2-producer` — pass it to
  `--actor` so the challenge's independence is checkable.
- Onboarding & Fulfillment spans this category's capabilities as the catalogue defines them;
...
`engine.cli gate --run <R> --root <ROOT> --category P2C2
