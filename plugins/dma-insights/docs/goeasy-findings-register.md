...
| GSY-25 | Cells closed as "no evidence" without an enrichment attempt, and the closure could be written from three places (a notebook consolidation, a synthesis record, a hand edit) — the gate trusted a two-cell flag. | `Absence_Claimed` had three unchecked writers; `is_declared_absent` read two cells; `open_stage(force=True)` bypassed every floors term; the primary question and the enrichment connectors had no gate term; `--tool` was free text. | One writer (`engine.cli absence`) proven by a Provenance row; `primary_unfired` and `absence_single_tool` blocking; `--tool` from `SEARCH_TOOLS`, `--subcap/--facet` required unless `--prelim`; validator rule 8; `open_stage` has no force; run-level density floors from `gold_reference.json`. `tests/skills/research_engine/test_absence_single_writer.py`. |
| GSY-26 | Report sections written before scoring and rendered past the gate; the gold floor would have failed Golden 1 itself; the report agents got the researcher's brief. | `narrative.write` skipped the preconditions when `run=None`; `reports.render` never ran them; `--force` swallowed readiness; a flat citation floor of 60 vs the reference's 47; `session_brief` routed by the substring `research-`. | Preconditions on every call; `--force` → `DRAFT_` no package accepts; floors scaled from the reference; the branded `.docx` shell pinned and bound; per-tier briefs (report, scoring, research); the gold gate wired into `assemble verify`. |
| GSY-27 | The workbook defaulted to the wrong structure every run. | `populate_workbook.py` built a second 10-sheet workbook; `assessment_runner.py` built a fresh 11-sheet `Workbook()`; both SKILL.md files told an agent to run them; no formatting, no validation lists. | Retired writers refuse and name the engine; `audit_skills.py` fails on a SKILL.md line that names one; number formats, data-validation lists and autofilters at write time; `deny_artefact_writes` PreToolUse hook. |
| GSY-28 | No driver: ten stages narrated by the conductor, most lanes dispatched "with the run id and the root", the handback never fed back, no stage timing anywhere, `--lanes 4` against a schedule that divided by 16, six hours with nothing recorded. | Orchestration lived in prose. | `engine.pipeline` (fourteen stages, done-predicates, gates, re-dispatch with the handback and blocking terms, `STAGE_*` rows with wall clock, resume, idempotence); `engine.cost record/report`; `agent_run.py --retries/--timing-out/--record-run`; one lane constant; briefs for every tier. `tests/acceptance/test_acceptance_pipeline.py`, `test_acceptance_issues_8_9.py`, `scripts/stress_pipeline_stub.py`. |
| GSY-29 | Ship-as-you-go stopped at a scheduler hop; promotion was a manual transport exercise; the plugin could not be invoked from a Routine or a plain session; a stale 0.9.12 install ran none of the gates the checkout published and nothing refused. | No connector reads or writes from the research engine; no `commands` key; no headless binding rule; the install check warned and repaired nothing. | The driver's two ingests and `ship_page.py --claim --verdicts-out` (exit 3 on a refused lease); `/dma-insights:run-assessment`; ROUTINES §2h `dma-research-intake` (reuses a recorded answer, never binds); `engine.cli start` refuses on STALE/DIVERGED/INCOMPLETE/DISABLED/MANIFEST_SPLIT and on a zip that predates its templates (`engine.template zip-guard`); the session brief refuses research work on those states. |
