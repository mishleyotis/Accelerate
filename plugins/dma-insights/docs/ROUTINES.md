...
(a) `python3 plugins/dma-insights/scripts/doctor.py` — the plugin, the Google identity, the agent inventory and the live connector roster. Not green, STOP and quote the failing row.
...
(d) ENRICHMENT CONNECTORS. Read this SESSION's toolset, not the Routine record: **Exa, Tavily, Firecrawl, Clay and Vibe-Prospecting/Explorium** must all be present as `mcp__<Name>__` tools. Any missing, STOP WITHOUT PREPARING ANYTHING and report exactly which this session carries against which it needs. The fix is a human attaching them on this Routine's own edit screen in the claude.ai routines UI — the connector browse list's Use buttons enable a connector for the ORG, not for a Routine (measured 2026-08-20). Do not proceed at reduced depth: a binding decided without the technographic and firmographic reads rests on less evidence than the entity deserves, and it is cheaper to stop than to research the wrong 851 cells.

State the result of all four in one line before moving on. A firing that stops here is a complete and correct firing. THIS IS THE ONE THING THAT PRECEDES STEP 0.5: a client the owner named still does not get researched on unproven tooling.
