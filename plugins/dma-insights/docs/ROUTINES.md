...
STEP 0 — VERIFY THE TOOLING. This container was provisioned before the session started by the environment setup script (plugins/dma-insights/scripts/bootstrap_session.sh, wired in the claude.ai/code environment settings together with the DMA_ROUTINE_SA_KEY_B64 variable): repo checkout, plugin install, service-account identity, connector path token. Run `claude plugin list` and the dma-insights doctor command (/dma-insights:doctor). Require: plugin dma-insights version >= 0.6.1 (the 47-agent roster plus the service-account access floor), doctor fully green including the live tool-roster check. THEN the access preflight, in this order: (a) `python3 plugins/dma-insights/scripts/drive_fetch.py check` — REQUIRED: the intake folder must answer the routine service account; if it fails, STOP and report its exact message (it names the one-time share that fixes it) — a synthesis that cannot reach the DMA drive folder does not start; (b) `python3 plugins/dma-insights/scripts/enrich_api.py check` — record which enrichment services are configured; missing ones DEGRADE their facets honestly (recorded not-run), they do not block production. claude.ai connectors, when a session happens to carry them, are a faster substitute — never a dependency; the scripts are the floor (docs/CONNECTORS.md). If the plugin is missing, stale, or the doctor fails, STOP and report exactly what is missing, naming bootstrap_session.sh and DMA_ROUTINE_SA_KEY_B64 so the fix is actionable — producing anything with degraded tooling is worse than producing nothing.
