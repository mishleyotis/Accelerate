...
| `research-challenger` | research | The Sonnet challenge pass over a converged category's syntheses — seven dimensions from the brief packet alone, one chained `engine.cli challenge` per cell; no web, no `get_evidence`, `engine.cli fetch` only for a non-verbatim excerpt | the driver (`engine.pipeline`), over `engine.brief challenge-batch`, after a category's floors gate converges and before it demands a challenge |
