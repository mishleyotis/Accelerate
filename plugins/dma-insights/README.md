...
Seventy-three in all: 30 producing agents in two tiers (six page producers
fanning out to twenty-four per-surface producers), the research tier
(`research-conductor`, the sixteen category researchers
`research-p1c1-producer` … `research-p4c4-producer` generated by
`scripts/gen_research_agents.py`, the `technographic-scanner`, the two
report producers and the `report-validator`), the scoring tier (four
pillar scorers `scoring-p1-producer` … `scoring-p4-producer` and the
`scoring-critic`, generated by `scripts/gen_scoring_agents.py`; they own
column D through `engine.assessment`, whose gate must PASS before a report
section can be written), three orchestration (`surface-producer`,
`package-vetter`, `page-consolidator`), four checkers, four enrichment,
three QA and three learning agents. The
routing authority is
`skills/dma-surface-production/05-lifecycle/routing.md`; the table lists
the ones invoked directly.
...
| `research-conductor` | a research engagement starts (entity + sub-vertical + evidence mode), or a run must resume | no — it ships the Drive folder, never the connector |
