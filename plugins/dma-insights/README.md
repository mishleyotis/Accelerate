...
Sixty-four in all: 30 producing agents in two tiers (six page producers
fanning out to twenty-four per-surface producers), the research tier
(`research-conductor` plus the sixteen category researchers
`research-p1c1-producer` … `research-p4c4-producer`, generated by
`scripts/gen_research_agents.py`), three orchestration
(`surface-producer`, `package-vetter`, `page-consolidator`), four
checkers, four enrichment, three QA and three learning agents. The
